...
QStringList WombatProperties::PopulateFileSystemProperties(TSK_FS_INFO* curfsinfo)
{
    ...
    else if(curfsinfo->ftype == TSK_FS_TYPE_ISO9660) // Byte offset's aren't working out too well right now.
    {
        ...
        for(p = iso->pvd; p!= NULL; p = p->next)
        {
            ...
            //sprintf(asc, "%i", (int)p->pvd.make_date.month);
            sprintf(asc, "%d%d/%d%d/%d%d%d%d %d%d:%d%d:%d%d GMT %d", (char)p->pvd.make_date.month[0], p->pvd.make_date.month[1], p->pvd.make_date.day[0], p->pvd.make_date.day[1], p->pvd.make_date.year[0], p->pvd.make_date.year[1], p->pvd.make_date.year[2], p->pvd.make_date.year[3], p->pvd.make_date.hour[0], p->pvd.make_date.hour[1], p->pvd.make_date.day[0], p->pvd.make_date.day[1], p->pvd.make_date.sec[0], p->pvd.make_date.sec[1], p->pvd.make_date.gmt_off);  
            //QString datestring = QString::fromLatin1(reinterpret_cast<char*>(p->pvd.make_date.month)) + "/" + QString::fromLatin1(reinterpret_cast<char*>(p->pvd.make_date.day)) + "/" + QString::fromLatin1(reinterpret_cast<char*>(p->pvd.make_date.year)) + " " + QString::fromLatin1(reinterpret_cast<char*>(p->pvd.make_date.hour)) + ":" + QString::fromLatin1(reinterpret_cast<char*>(p->pvd.make_date.min)) + ":" + QString::fromLatin1(reinterpret_cast<char*>(p->pvd.make_date.sec)) + " GMT " + QString::number(p->pvd.make_date.gmt_off);
