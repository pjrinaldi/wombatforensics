...
void ParseDirectoryStructure(ForImg* curimg, uint32_t curstartsector, uint8_t ptreecnt)
{
	...
	curinode = ParseNtfsDirectory(curimg, curstartsector, ptreecnt, 5, 0, "", "");
        ParseNtfsOrphans(curimg, curstartsector, ptreecnt, curinode);
    }
...
quint64 ParseNtfsDirectory(ForImg* curimg, uint32_t curstartsector, uint8_t ptreecnt, quint64 parentntinode, quint64 parinode, QString parfilename, QString parlayout)
{
    ...
    */
    ...
    QList<uint16_t> indxallocoffset;
    QList<uint16_t> indxalloclength;
	...
	else if(attrtype == 0xa0) // $INDEX_ALLOCATION - ALWAYS NON_RESIDENT
	{
	    ...
	    for(int i=0; i < layout.split(";", Qt::SkipEmptyParts).count(); i++)
	    {
		indxallocoffset.append(layout.split(";", Qt::SkipEmptyParts).at(i).split(",").at(0).toULongLong() / bytespercluster);
		indxalloclength.append(layout.split(";", Qt::SkipEmptyParts).at(i).split(",").at(1).toULongLong() / bytespercluster);
	    }
	}
	else if(attrtype == 0xffffffff)
	    break;
...
void ParseNtfsOrphans(ForImg* curimg, uint32_t curstartsector, uint8_t ptreecnt, quint64 curinode)
{
    // mftlayout, mftentrybytes, bytespercluster, curstartsector
    QString mftlayout = "";
    uint16_t mftentrybytes = 0;
    uint32_t bytespercluster = 0;
    quint64 maxmftentries = 0;
