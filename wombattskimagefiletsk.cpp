...
/**
 * \file WombatTskImageFileTsk.cpp
 ...
 */
void WombatTskImageFileTsk::closeFs(std::pair<uint64_t, TSK_FS_INFO*> pair)
{
 ...
 */
int WombatTskImageFileTsk::open()
{
    if (!m_images.empty()) {
        close();
    }
    ...
    if (images.empty()) {
        LOGERROR(L"WombatTskImageFileTsk::open: Error getting image names from ImgDB");
        return -1;
    }
 ...
 * @returns -1 on error.
 */
int WombatTskImageFileTsk::openImages(const TSK_IMG_TYPE_ENUM imageType,
                                const unsigned int sectorSize)
{
    ...
    if (m_img_info == NULL)
    {
        ...
        logMessage << L"WombatTskImageFileTsk::openImages - Error with tsk_img_open: " << tsk_error_get() << std::endl;
...
void WombatTskImageFileTsk::close()
{
    if (m_img_info) {
    ...
    std::for_each(m_openFs.begin(), m_openFs.end(), (&WombatTskImageFileTsk::closeFs));
}
 ...
 */
int WombatTskImageFileTsk::getSectorData(const uint64_t sect_start,
 ...
 */
int WombatTskImageFileTsk::getByteData(const uint64_t byte_start,
                                ...
                                char *buffer)
{
    ...
    if (retval == -1) {
        ...
        message << L"WombatTskImageFileTsk::getByteData - tsk_img_read -- start: "
...
int WombatTskImageFileTsk::extractFiles()
{
    // @@@ Add Sanity check that DB is empty
    if (m_img_info == NULL) {
        LOGERROR(L"WombatTskImageFileTsk::extractFiles: Images not open yet\n");
        return 1;
    }
...
int WombatTskImageFileTsk::openFile(const uint64_t fileId)
{
    if (m_img_info == NULL) {
        if (open() != 0)
            return -1;
    ...
    {
        LOGERROR(L"WombatTskImageFileTsk::openFile - Error getting file identifiers.\n");
        return -1;
    }
    ...
    if (fsInfo == NULL)
    {
        ...
        if (fsInfo == NULL)
        {
            ...
            errorMsg << L"WombatTskImageFileTsk::openFile - Error opening file system : " << tsk_error_get();
    ...
    {
        std::wstringstream errorMsg;
        errorMsg << L"WombatTskImageFileTsk::openFile - Error opening file : " << tsk_error_get();
        LOGERROR(errorMsg.str());
        return -1;
    }
    ...
    {
        std::wstringstream msg;
        msg << L"WombatTskImageFileTsk::openFile - Error getting attribute : " << tsk_error_get();
        LOGERROR(msg.str());
        return -1;
    }

    WombatTskImageFileTsk::OPEN_FILE * openFile = new WombatTskImageFileTsk::OPEN_FILE();
...
int WombatTskImageFileTsk::readFile(const int handle,
                              const TSK_OFF_T byte_offset,
                              const size_t byte_len,
                              char * buffer)
{
    WombatTskImageFileTsk::OPEN_FILE * openFile = m_openFiles[handle];

    if (openFile == NULL || openFile->fsFile == NULL)
    {
        std::wstringstream errorMsg;
        errorMsg << L"WombatTskImageFileTsk::readFile - Either OPEN_FILE or TSK_FS_FILE is null." << std::endl;
        LOGERROR(errorMsg.str());
        return -1;
    }
    ...
    {
        std::wstringstream errorMsg;
        errorMsg << L"WombatTskImageFileTsk::readFile - Error reading file (FS_OFFSET: "
...
int WombatTskImageFileTsk::closeFile(const int handle)
{
    ...
    {
        std::wstringstream errorMsg;
        errorMsg << L"WombatTskImageFileTsk::closeFile - Either OPEN_FILE ot TSK_FS_FILE is null." << std::endl;
        LOGERROR(errorMsg.str());
        return -1;
    }
...
std::vector<std::wstring> WombatTskImageFileTsk::getFileNamesW() const
{
...
int WombatTskImageFileTsk::open(const TSK_TCHAR *imageFile,
...
int WombatTskImageFileTsk::open(const int numberOfImages,
...
int WombatTskImageFileTsk::open(const std::string &imageFile,
                          const TSK_IMG_TYPE_ENUM imageType,
                          const unsigned int sectorSize)
{
    if (!m_images.empty()) {
...
int WombatTskImageFileTsk::open(const std::wstring &imageFile,
                          const TSK_IMG_TYPE_ENUM imageType,
                          const unsigned int sectorSize)
{
...
int WombatTskImageFileTsk::open(const std::vector<std::string> &imageFile,
                          const TSK_IMG_TYPE_ENUM imageType,
                          const unsigned int sectorSize)
{
    if (!m_images.empty()) {
...
int WombatTskImageFileTsk::open(const std::vector<std::wstring> &imageFile,
                          const TSK_IMG_TYPE_ENUM imageType,
                          const unsigned int sectorSize)
{
    if (!m_images.empty()) {
