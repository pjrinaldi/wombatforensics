...
void SideLoad(Node* parentnode)
{
    unsigned long long parentaddr = parentnode->nodevalues.at(0).toString().split("-").last().mid(1).toULongLong();
    int curpart = parentnode->nodevalues.at(0).toString().split("-").at(2).mid(1).toInt();
    ...
    QString tmpstr = "";
        QDir eviddir = QDir(wombatvariable.tmpmntpath);
        QStringList evidfiles = eviddir.entryList(QStringList("*.evid.*" + parentnode->nodevalues.at(0).toString().split("-").at(0).mid(1)), QDir::NoSymLinks | QDir::Files);
        // could get rid fo the evidfile loop if i simply use the evid # from the parentaddr split...
        //for(int j = 0; j < evidfiles.count(); j++)
        //{
            QStringList curfiles = GetChildFiles(evidfiles.at(0).split(".evid").at(0) + ".p" + QString::number(curpart) + "*.a" + QString::number(parentaddr));
            if(curfiles.count() > 0)
            {
                QFile childfile;
