...
#include "/usr/local/include/tsk/framework/utilities/TskModuleDev.h"
...
namespace
{
    const char *MODULE_NAME = "tskTestModule";
    const char *MODULE_DESCRIPTION = "Perform a Test Function on files";
    const char *MODULE_VERSION = "0.0.1";

    /**
    * Return the filename prefixed by PJR-.
    *
    * @param pFile A TskFile object corrersponding to a file.
    * @return The filename with my prefix.
    */
    std::string calculateName(TskFile *pFile)
    {
        string tmpstring = "pjr-";
        tmpstring = tmpstring + pFile->getName();
        return tmpstring;
    }
    /*
    ...
    */
    TskModule::Status TSK_MODULE_EXPORT run(TskFile *pFile)
    {
        ...
        try
        {
            ...
            // Create my string function
            string mystring = calculateName(pFile);

            // Post the value to the blackboard.
            pFile->addGenInfoAttribute(TskBlackboardAttribute(TSK_TAG_FILE, MODULE_NAME, "", mystring));
